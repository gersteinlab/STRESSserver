...
'''
An older version of this script takes a PDB structure as input, and generates 
a simplified PDB that does not contain residues such as "AGLY/BGLY", and it 
converts modified residues to their appropriate names --- for ex:

	HETATM ... 	SEP ....
	   would get changed to:
	ATOM ...  	SER  ...

However, this new version simply removes HETATM lines (this needs to be the case in 
order for MMTK to produce fnm files correctly - HETATMs will cause MMTK to fail)
...
##  
##  usage:
##  python just_make_simplified_pdb_and_rm_HETATM.py 1J3H.pdb 1J3H_SIMPLIFIED.pdb
...
list_of_possible_modified_residue_names = ["PTR", "PSR", "SEP", "PTH", "PHY", "PHS", "PHT", "MSE", "MSO", "DAL", "NAL", "02K", "HR7", "2MR", "ASA", "PHD", "CSO", "SCH", "OCY", "SCH", "CMT", "CSS", "CAS", "CME", "CSX", "OCS", "CSW", "03Y", "PCA", "CGU", "EME", "B3E", "SAR", "56A", "MK8", "NLE", "MLE", "ALY", "LGY", "BTK", "PRK", "MLY", "LLP", "M3L", "MLZ", "PRS", "MEA", "0BN", "3PX", "HYP", "TPO", "TTQ", "HT7", "PTR", "TYS", "LE1", "UNK"]
	...
	if line[0:6] == "HETATM":
		...
		for c in res_name_pre:
			if c.isalnum():
				num_alpha_numeric_chars += 1
		if num_alpha_numeric_chars > 3:  ## then we may have something such as "BARG"
			print >> sys.stderr, "NON-standard res_name_pre here:  |" + str(res_name_pre) + "|\n" + str(line)
			#print "here:  " + res_name_pre[0:1] + "\n\n\n"
			if res_name_pre[0:1] is not "A":  ## ie -- take only the lines w/"AARG", and not "BARG"
				is_normal_res_name = "no"
			else: ## then we have "AARG"
				res_name_rev = " " + str(res_name_pre[1:4])
				print >> sys.stderr, "res_name_rev:  |" + str(res_name_rev) + "|"
				res_name = res_name_rev
		else:
			res_name = res_name_pre
		...
		if (is_standard_res_number == "yes")  and  (is_normal_res_name == "yes"):
			...
			print >> sys.stderr, "NON-standard full line for pdb file:  " + str(pdb_file) + "   \n" + str(res_id_pre) + "\n" + str(line) + "\n"
